...
/**
  ...
  */
class EqRequestControllerSpec
  extends TestKit(ActorSystem("EqRequestControllerSpec"))
    with WordSpecLike
    with Matchers
  ...
  "EqRequestController" can {
    val eRCActor = system.actorOf(EqRequestController.props(system))
    "accept requests, stash or process them after" in {
      ...
      expectMsgPF(Duration(1, SECONDS)) {
